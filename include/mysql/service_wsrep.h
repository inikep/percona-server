...
#include "mysql/status_var.h" /* MYSQL_THD */
...
/* Must match to definition in sql/mysqld.h, int64 == long long int */
typedef int64_t int64;
typedef int64 query_id_t;

/* note: bool == char replaced in below */
extern struct wsrep_service_st {
  bool                     (*get_wsrep_recovery_func)();
  ...
  bool                     (*wsrep_on_func)(const MYSQL_THD thd);
  bool                     (*wsrep_is_ready_func)();
  ...
  void                        (*wsrep_lock_thd_query_func)(MYSQL_THD thd, bool lock);
  const char *                (*wsrep_thd_query_func)(MYSQL_THD thd);
  ...
  bool                     (*wsrep_thd_is_aborting_func)(const MYSQL_THD thd);
  void                        (*wsrep_set_data_home_dir_func)(const char *data_dir);
  bool                     (*wsrep_thd_is_BF_func)(MYSQL_THD thd, bool sync);
  bool                     (*wsrep_thd_is_local_func)(const MYSQL_THD thd);
  bool                     (*wsrep_thd_is_async_slave_func)(const MYSQL_THD thd);
  ...
  bool                     (*wsrep_thd_bf_abort_func)(MYSQL_THD bf_thd,
                                                         MYSQL_THD victim_thd,
                                                         bool signal);
  bool                     (*wsrep_thd_order_before_func)(const MYSQL_THD left, const MYSQL_THD right);
  void                        (*wsrep_handle_SR_rollback_func)(MYSQL_THD BF_thd, MYSQL_THD victim_thd);
  bool                     (*wsrep_thd_skip_locking_func)(const MYSQL_THD thd);
  const char*                 (*wsrep_get_sr_table_name_func)();
  bool                     (*wsrep_get_debug_func)();
  void                        (*wsrep_thd_xid)(const MYSQL_THD thd, void *xid, size_t xid_size);
  void                        (*wsrep_commit_ordered_func)(MYSQL_THD thd);
  bool                        (*wsrep_thd_set_wsrep_aborter_func)(MYSQL_THD bf_thd, MYSQL_THD thd);
  void                        (*wsrep_thd_set_PA_unsafe_func)(MYSQL_THD thd);
 } *wsrep_service;
...
#define wsrep_is_ready() wsrep_service->wsrep_is_ready_func()
...
#define wsrep_lock_thd_query(T) wsrep_service->wsrep_lock_thd_query_func(T,B)
...
#define wsrep_thd_is_async_slave(T) wsrep_service->wsrep_thd_is_async_slave_func(T)
...
#define wsrep_thd_xid() wsrep_service->wsrep_thd_xid(T,X,S)
...
extern "C" void wsrep_lock_thd_query(MYSQL_THD thd, bool lock);
extern "C" const char *wsrep_thd_query(MYSQL_THD thd);
...
extern "C" bool wsrep_on(const MYSQL_THD thd);
/* Return true if node is in ready state */
...
extern "C" bool wsrep_thd_is_local(const MYSQL_THD thd);
/* Return true if thd is in asynchronous slave */
extern "C" bool wsrep_thd_is_async_slave(const MYSQL_THD thd);
/* Return true if thd is in high priority mode */
/* todo: rename to is_high_priority() */
extern "C" bool wsrep_thd_is_applying(const MYSQL_THD thd);
...
extern "C" bool wsrep_thd_is_toi(const MYSQL_THD thd);
/* Return true if thd is in replicating TOI mode */
extern "C" bool wsrep_thd_is_local_toi(const MYSQL_THD thd);
/* Return true if thd is in RSU mode */
extern "C" bool wsrep_thd_is_in_rsu(const MYSQL_THD thd);
/* Return true if thd is in BF mode, either high_priority or TOI */
extern "C" bool wsrep_thd_is_BF(MYSQL_THD thd, bool sync);
/* Return true if thd is streaming */
extern "C" bool wsrep_thd_is_SR(const MYSQL_THD thd);
...
/* BF abort victim_thd */
extern "C" bool wsrep_thd_bf_abort(MYSQL_THD bf_thd,
                                      MYSQL_THD victim_thd,
                                      bool signal);
/* Return true if left thd is ordered before right thd */
extern "C" bool wsrep_thd_order_before(const MYSQL_THD left, const MYSQL_THD right);
...
extern "C" bool wsrep_thd_skip_locking(const MYSQL_THD thd);
...
extern "C" void wsrep_thd_xid(const MYSQL_THD thd, void *xid, size_t xid_size);
extern "C" void wsrep_commit_ordered(MYSQL_THD thd);

extern "C" void wsrep_thd_auto_increment_variables(THD* thd,
                                                   unsigned long long* offset,
                                                   unsigned long long* increment);
/* declare THD as unsafe for parallel applying */
