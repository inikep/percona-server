...
@return 0 on success */
static int i_s_dict_fill_tablespaces_encryption(THD* thd,
                                                fil_space_t* space,
                                                TABLE* table_to_fill)
{
  Field** fields;
  ...
  OK(fields[TABLESPACES_ENCRYPTION_ENCRYPTION_SCHEME]->store(status.scheme, true));
  OK(fields[TABLESPACES_ENCRYPTION_KEYSERVER_REQUESTS]->store(status.keyserver_requests, true));
  OK(fields[TABLESPACES_ENCRYPTION_MIN_KEY_VERSION]->store(status.min_key_version, true));
  OK(fields[TABLESPACES_ENCRYPTION_CURRENT_KEY_VERSION]->store(status.current_key_version, true));
  OK(fields[TABLESPACES_ENCRYPTION_CURRENT_KEY_ID]->store(status.key_id, true));
  OK(fields[TABLESPACES_ENCRYPTION_ROTATING_OR_FLUSHING]->store(status.rotating || status.flushing, true));
  
  if (status.rotating) {
    fields[TABLESPACES_ENCRYPTION_KEY_ROTATION_PAGE_NUMBER]->set_notnull();
    OK(fields[TABLESPACES_ENCRYPTION_KEY_ROTATION_PAGE_NUMBER]->store(status.rotate_next_page_number, true));
    fields[TABLESPACES_ENCRYPTION_KEY_ROTATION_MAX_PAGE_NUMBER]->set_notnull();
    OK(fields[TABLESPACES_ENCRYPTION_KEY_ROTATION_MAX_PAGE_NUMBER]->store(status.rotate_max_page_number, true));
  } else {
...
@return 0 on success */
static int i_s_tablespaces_encryption_fill_table(THD* thd,
                                                 TABLE_LIST* tables,	/*!< in/out: tables to fill */
                                                 Item* ) /*!< in: condition (not used) */
{
  btr_pcur_t pcur;
...
static
int
innodb_tablespaces_encryption_init(void* p) /*!< in/out: table schema object */
{
  ST_SCHEMA_TABLE*	schema;
