...
static PSI_thread_info all_innodb_threads[] = {
    PSI_THREAD_KEY(srv_slowrm_thread, "ib_srv_slowrm", PSI_FLAG_SINGLETON, 0,
                   PSI_DOCUMENT_ME),
...
static SHOW_VAR innodb_status_variables[] = {
    ...
    {"pages_written", (char *)&export_vars.innodb_pages_written, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"purge_pending", (char *)&export_vars.innodb_purge_pending, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    ...
    {"row_lock_current_waits",
     ...
     SHOW_SCOPE_GLOBAL},
    {"row_lock_deadlocks", (char *)&export_vars.innodb_lock_deadlocks,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    ...
    {"row_lock_waits", (char *)&export_vars.innodb_row_lock_waits, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"row_lock_wait_timeouts", (char *)&export_vars.innodb_lock_wait_timeouts,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"row_recreations", (char *)&export_vars.innodb_row_recreations, SHOW_LONG,
     SHOW_SCOPE_GLOBAL},
    {"row_recreation_steps", (char *)&export_vars.innodb_row_recreation_steps,
     SHOW_LONG, SHOW_SCOPE_GLOBAL},
...
static int innobase_start_trx_and_assign_read_view(
    ...
    THD *thd,                   /* in: MySQL thread handle of the
                                 user for whom the transaction should
                                 be committed */
    snapshot_info_st *ss_info); /* in: Binlog snapshot info like binlog pos,
                                 gtid etc */
    ...
    bool force_digest)  /*!< in: always show query digest */
{
  ...
  char *msg = thd_security_context_internal(thd, buffer, sizeof buffer,
...
void ha_innobase::encode_autogenerated_pk(const longlong row_num,
                                          uchar *record) {
  // see dict_table_get_next_table_sess_row_id() for reference
  for (int i = 0; i < 6; i++) {
    record[i] = (uchar)(row_num >> ((5 - i) * 8));
  }
}
...
static int innodb_show_status(handlerton *hton, THD *thd,
  ...
  str = (char *)my_malloc(PSI_INSTRUMENT_ME, flen + 1, MYF(0));
  if (!str) {
    ...
    my_off_t *binlog_pos)  /*!< out: Last valid binlog pos */
{
  ...
  if (!binlog_max_gtid || trx_sys_get_mysql_bin_log_max_gtid(gtid_buf)) {
    return;
  }
