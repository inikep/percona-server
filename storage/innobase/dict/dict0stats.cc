                            ...
                            const dict_table_t *src) /*!< in: source table */
{
  ...
  for (dst_idx = dst->first_index(), src_idx = src->first_index();
       dst_idx != NULL; dst_idx = dst_idx->next(),
      (src_idx != NULL && (src_idx = src_idx->next()))) {
    ...
    if (!INDEX_EQ(src_idx, dst_idx)) {
      for (src_idx = src->first_index(); src_idx != NULL;
    ...
    mtr_t *mtr)                      /*!< in/out: mini-transaction */
{
  ...
  *total_pages = 0;
  ...
  for (; btr_pcur_is_on_user_rec(&pcur);
    ...
    where rec resides. */
    if (prev_rec != NULL && page_align(rec) != page_align(prev_rec)) {
    ...
    if (level == 0 &&
      ...
      if (rec_is_last_on_page && !prev_rec_is_copied && prev_rec != NULL) {
        /* copy prev_rec */
  ...
  for (ulint n_prefix = dict_index_get_n_unique(index); n_prefix >= 1;
    ...
    if (data->level == 1) {
      ...
      n_ordinary_leaf_pages =
          index->stat_n_leaf_pages * data->n_leaf_pages_to_analyze /
    ...
    index->stat_n_sample_sizes[n_prefix - 1] = data->n_leaf_pages_to_analyze;
    ...
    DEBUG_PRINTF(
        ...
        __func__, index->stat_n_diff_key_vals[n_prefix - 1], n_prefix,
        index->stat_n_leaf_pages, data->n_diff_on_level, data->n_recs_on_level,
        data->n_diff_all_analyzed_pages, data->n_leaf_pages_to_analyze);
  }
}
...
static dberr_t dict_stats_save_index_stat(dict_index_t *index, lint last_update,
  ...
  if (sample_size != NULL) {
    ...
    pars_info_add_literal(pinfo, "sample_size", NULL, UNIV_SQL_NULL,
                          DATA_FIXBINARY, 0);
  }
...
static dberr_t dict_stats_save(dict_table_t *table_orig,
  ...
  for (it = indexes.begin(); it != indexes.end(); ++it) {
    ...
    if (only_for_index != NULL && index->space != only_for_index->m_space_id &&
  ...
  for (cnode = static_cast<que_common_t *>(node->select_list), i = 0;
    ...
    switch (i) {
      case 0: /* mysql.innodb_index_stats.index_name */
        ...
        for (index = table->first_index(); index != NULL;
             index = index->next()) {
