...
static void srv_init(void) {
  ...
  and so mutex creation is needed. */
  {
    ...
    srv_slowrm_event = os_event_create();
...
*/
static void srv_printf_locks_and_transactions(FILE *file, bool include_trxs) {
...
bool srv_printf_innodb_monitor(
    ...
    bool nowait,       /*!< in: whether to wait for the
                        lock_sys_t:: mutex */
    bool include_trxs) /*!< in: include per-transaction output */
{
  ulint n_reserved;
  bool ret;
