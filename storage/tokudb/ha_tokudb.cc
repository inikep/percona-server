...
//
static inline uint32_t get_fixed_field_size(KEY_AND_COL_INFO* kc_info,
...
static inline uint32_t get_len_of_offsets(KEY_AND_COL_INFO* kc_info,
                                          TABLE_SHARE* table_share,
                                          uint keynr) {
    uint len = 0;
    for (uint i = 0; i < table_share->fields; i++) {
...
static int allocate_key_and_col_info(TABLE_SHARE* table_share,
    ...
    for (uint i = 0; i < MAX_KEY + 1; i++) {
        error = bitmap_init(
            &kc_info->key_filters[i], NULL, table_share->fields, false);
        if (error) {
            goto exit;
        }
    }
    ...
    kc_info->multi_ptr = tokudb::memory::multi_malloc(
        ...
        &kc_info->field_types,
        (uint)(table_share->fields * sizeof(uint8_t)),
        &kc_info->field_lengths,
        (uint)(table_share->fields * sizeof(uint16_t)),
        &kc_info->length_bytes,
        (uint)(table_share->fields * sizeof(uint8_t)),
        &kc_info->blob_fields,
        (uint)(table_share->fields * sizeof(uint32_t)),
        NullS);
...
void TOKUDB_SHARE::init(const char* table_name) {
    ...
    tokudb_split_dname(
        table_name, _database_name, _table_name, tmp_dictionary_name);
        ...
        for (uint i = 0; i < sizeof(key_file) / sizeof(key_file[0]); i++) {
            if (key_file[i]) {
                TOKUDB_TRACE_FOR_FLAGS(
                    TOKUDB_DEBUG_OPEN, "dbclose:%p", key_file[i]);
...
void TOKUDB_SHARE::update_row_count(THD* thd,
                                    uint64_t added,
                                    uint64_t deleted,
                                    uint64_t updated) {
...
static inline bool do_ignore_flag_optimization(THD* thd,
                                               TABLE* table,
                                               bool opt_eligible) {
    bool do_opt = false;
...
static int smart_dbt_callback_rowread_ptquery(DBT const* key,
                                              DBT const* row,
                                              void* context) {
...
//
static int smart_dbt_callback_rowread(DBT const* key,
                                      DBT const* row,
                                      void* context) {
    int error = 0;
    SMART_DBT_INFO info = (SMART_DBT_INFO)context;
...
// Smart DBT callback function in case where we do NOT have a covering index
//
static int smart_dbt_callback_ir_rowread(DBT const* key,
                                         DBT const* row,
                                         void* context) {
    INDEX_READ_INFO ir_info = (INDEX_READ_INFO)context;
    ir_info->cmp = ir_info->smart_dbt_info.ha->prefix_cmp_dbts(
...
#define SET_PRELOCK_FLAG(flg)                                             \
    ((flg) | (range_lock_grabbed                                          \
                  ? (use_write_locks ? DB_PRELOCKED_WRITE : DB_PRELOCKED) \
                  : 0))
...
//
static ulonglong retrieve_auto_increment(uint16 type,
                                         uint32 offset,
                                         const uchar* record) {
    const uchar* key;               /* Key */
...
// in the debugger.
//
void set_key_filter(MY_BITMAP* key_filter,
                    KEY* key,
                    TABLE* table,
            ...
            if (curr_skip_offset == curr_field_offset) {
                ...
                switch (toku_type) {
                    ...
                    case toku_type_fixstring:
                        if (key->key_part[curr_key_index].length ==
                            field->field_length) {
...
static inline uchar* pack_fixed_field(uchar* to_tokudb,
...
static inline void unpack_var_field(uchar* to_mysql,
    ...
    //
    memcpy(
        to_mysql + mysql_length_bytes, from_tokudb_data, from_tokudb_data_len);
}

static uchar* pack_toku_field_blob(uchar* to_tokudb,
                                   const uchar* from_mysql,
                                   Field* field) {
    ...
    if (dest_val != NULL) {
        if (!is_key_clustering(desc_size) || src_val->size == 0) {
            ...
            dest_val->size = pack_clustering_val_from_desc(
                buff, row_desc, desc_size, src_val);
...
static int generate_row_for_del(DB* dest_db,
                                DB* src_db,
    ...
    return tokudb_generate_row(
        dest_db, src_db, dest_key, NULL, src_key, src_val);
}

static int generate_row_for_put(DB* dest_db,
                                DB* src_db,
                                DBT_ARRAY* dest_key_arrays,
    ...
    DBT* dest_key = &dest_key_arrays->dbts[0];
    DBT* dest_val =
        (dest_val_arrays == NULL) ? NULL : &dest_val_arrays->dbts[0];
    return tokudb_generate_row(
        dest_db, src_db, dest_key, dest_val, src_key, src_val);
}
...
ha_tokudb::ha_tokudb(handlerton* hton, TABLE_SHARE* table_arg)
    ...
    for (uint32_t i = 0;
         i < sizeof(mult_key_dbt_array) / sizeof(mult_key_dbt_array[0]);
         i++) {
        toku_dbt_array_init(&mult_key_dbt_array[i], 1);
    }
    for (uint32_t i = 0;
         i < sizeof(mult_rec_dbt_array) / sizeof(mult_rec_dbt_array[0]);
         i++) {
...
int ha_tokudb::open_main_dictionary(const char* name,
                                    bool is_read_only,
                                    DB_TXN* txn) {
    int error;
    char* newname = NULL;
    ...
    error = share->file->open(
        share->file, txn, newname, NULL, DB_BTREE, open_flags, 0);
    if (error) {
        goto exit;
    }

    TOKUDB_HANDLER_TRACE_FOR_FLAGS(
        TOKUDB_DEBUG_OPEN, "open:%s:file=%p", newname, share->file);
...
// be a primary key
//
int ha_tokudb::open_secondary_dictionary(DB** ptr,
                                         KEY* key_info,
                                         const char* name,
                                         bool is_read_only,
                                         DB_TXN* txn) {
    ...
    TOKUDB_HANDLER_TRACE_FOR_FLAGS(
        TOKUDB_DEBUG_OPEN, "open:%s:file=%p", newname, *ptr);
cleanup:
    if (error) {
        if (*ptr) {
...
static int initialize_col_pack_info(KEY_AND_COL_INFO* kc_info,
                                    TABLE_SHARE* table_share,
                                    uint keynr) {
    int error = ENOSYS;
    //
    ...
    }
    {
        ...
        // set up the mcp_info
        //
        kc_info->mcp_info[keynr].fixed_field_size =
            get_fixed_field_size(kc_info, table_share, keynr);
        kc_info->mcp_info[keynr].len_of_offsets =
            get_len_of_offsets(kc_info, table_share, keynr);
...
static int initialize_key_and_col_info(TABLE_SHARE* table_share,
                                       TABLE* table,
                                       KEY_AND_COL_INFO* kc_info,
    ...
    for (uint i = 0; i < table_share->keys + tokudb_test(hidden_primary_key);
         i++) {
        //
        ...
        if (!(i == primary_key && hidden_primary_key)) {
            if (i == primary_key) {
                set_key_filter(&kc_info->key_filters[primary_key],
                               &table_share->key_info[primary_key],
                               table,
                               true);
            } else {
                set_key_filter(&kc_info->key_filters[i],
    ...
    for (uint curr_index = 0; curr_index < table_share->keys; curr_index++) {
        ...
        for (uint i = 0; i < curr_key_info->user_defined_key_parts; i++) {
            uint16 curr_field_index =
                curr_key_info->key_part[i].field->field_index;
            if (!bitmap_is_set(&kc_info->key_filters[curr_index],
                               curr_field_index)) {
                ret_val = false;
                goto exit;
            }
...
int ha_tokudb::initialize_share(const char* name, int mode) {
    ...
    if (thd_sql_command(thd) == SQLCOM_CREATE_TABLE && trx &&
        trx->sub_sp_level) {
    ...
    error = initialize_key_and_col_info(
        table_share, table, &share->kc_info, hidden_primary_key, primary_key);
    if (error) {
        goto exit;
    }
    ...
    // initialize cardinality info from the status dictionary
    {
        ...
        error = tokudb::get_card_from_status(
            share->status_block, txn, rec_per_keys, rec_per_key);
        if (error) {
...
int ha_tokudb::open(const char* name, int mode, uint test_if_locked) {
    ...
    alloc_ptr =
        tokudb::memory::multi_malloc(MYF(MY_WME),
                                     &key_buff,
                                     max_key_length,
                                     &key_buff2,
                                     max_key_length,
                                     ...
                                     table_share->fields * sizeof(uint32_t),
                                     &var_cols_for_query,
                                     table_share->fields * sizeof(uint32_t),
                                     NullS);
    if (alloc_ptr == NULL) {
    ...
    alloced_rec_buff_length =
        table_share->rec_buff_length + table_share->fields;
    rec_buff =
...
int ha_tokudb::estimate_num_rows(DB* db, uint64_t* num_rows, DB_TXN* txn) {
    ...
    if (txn == NULL) {
        error =
            txn_begin(db_env, 0, &txn_to_use, DB_READ_UNCOMMITTED, ha_thd());
        if (error)
            goto cleanup;
        do_commit = true;
    } else {
...
int ha_tokudb::write_to_status(DB* db,
                               HA_METADATA_KEY curr_key_data,
                               void* data,
                               uint size,
                               DB_TXN* txn) {
    return write_metadata(
        db, &curr_key_data, sizeof curr_key_data, data, size, txn);
}

int ha_tokudb::remove_from_status(DB* db,
                                  HA_METADATA_KEY curr_key_data,
                                  DB_TXN* txn) {
    return remove_metadata(db, &curr_key_data, sizeof curr_key_data, txn);
}

int ha_tokudb::remove_metadata(DB* db,
                               void* key_data,
                               uint key_size,
                               DB_TXN* transaction) {
    int error;
...
//
int ha_tokudb::write_metadata(DB* db,
                              void* key_data,
                              uint key_size,
                              void* val_data,
                              uint val_size,
...
int ha_tokudb::verify_frm_data(const char* frm_name, DB_TXN* txn) {
    ...
    error = share->status_block->getf_set(share->status_block,
                                          txn,
                                          0,
                                          &key,
...
bool ha_tokudb::fix_rec_buff_for_blob(ulong length) {
    if (!rec_buff || (length > alloced_rec_buff_length)) {
        uchar* newptr = (uchar*)tokudb::memory::realloc(
            (void*)rec_buff, length, MYF(MY_ALLOW_ZERO_PTR));
        if (!newptr)
            return 1;
        rec_buff = newptr;
...
int ha_tokudb::pack_row_in_buff(DBT* row,
                                const uchar* record,
                                uint index,
                                uchar* row_buff) {
...
int ha_tokudb::pack_old_row_for_update(DBT* row,
                                       const uchar* record,
                                       uint index) {
    return pack_row_in_buff(row, record, index, rec_update_buff);
}

int ha_tokudb::unpack_blobs(uchar* record,
    ...
    if (num_bytes > num_blob_bytes) {
        ptr = (uchar*)tokudb::memory::realloc(
            (void*)blob_buff, num_bytes, MYF(MY_ALLOW_ZERO_PTR));
        if (ptr == NULL) {
            error = ENOMEM;
            goto exit;
        }
...
//
int ha_tokudb::unpack_row(uchar* record,
                          DBT const* row,
                          DBT const* key,
                          uint index) {
    //
    ...
    if (unpack_entire_row) {
        ...
        for (uint i = 0; i < table_share->fields; i++) {
            ...
            if (is_fixed_field(&share->kc_info, i)) {
                fixed_field_ptr =
                    unpack_fixed_field(record + field_offset(field, table),
                                       fixed_field_ptr,
                                       share->kc_info.field_lengths[i]);
            }
            //
        ...
        error = unpack_blobs(record,
                             var_field_data_ptr,
                             row->size - (uint32_t)(var_field_data_ptr -
    ...
    //
        ...
        if (read_blobs) {
            ...
            error = unpack_blobs(
                ...
                var_field_data_ptr,
                row->size -
                    (uint32_t)(var_field_data_ptr - (const uchar*)row->data),
                true);
            if (error) {
                goto exit;
            }
...
uint32_t ha_tokudb::place_key_into_mysql_buff(KEY* key_info,
                                              uchar* record,
                                              uchar* data) {
    ...
    for (; key_part != end; key_part++) {
        ...
        pos =
            unpack_toku_key_field(record + field_offset(key_part->field, table),
                                  pos,
                                  key_part->field,
                                  key_part->length);
    }
...
void ha_tokudb::unpack_key(uchar* record, DBT const* key, uint index) {
    ...
    bytes_read =
        place_key_into_mysql_buff(&table->key_info[index], record, pos);
    if ((index != primary_key) && !hidden_primary_key) {
        //
        // also unpack primary key
        //
        place_key_into_mysql_buff(
            &table->key_info[primary_key], record, pos + bytes_read);
    }
}

uint32_t ha_tokudb::place_key_into_dbt_buff(KEY* key_info,
                                            uchar* buff,
                                            const uchar* record,
                                            bool* has_null,
                                            int key_length) {
...
DBT* ha_tokudb::create_dbt_key_from_key(DBT* key,
                                        KEY* key_info,
                                        uchar* buff,
                                        const uchar* record,
                                        bool* has_null,
    ...
    *tmp_buff++ = inf_byte;
    ...
    size += place_key_into_dbt_buff(
        key_info, tmp_buff, record, has_null, key_length);
    if (!dont_pack_pk) {
        tmp_buff = buff + size;
        if (hidden_primary_key) {
...
//      the parameter key
//
DBT* ha_tokudb::create_dbt_key_from_table(DBT* key,
                                          uint keynr,
                                          uchar* buff,
                                          const uchar* record,
                                          bool* has_null,
    ...
    DBUG_RETURN(create_dbt_key_from_key(key,
...
DBT* ha_tokudb::create_dbt_key_for_lookup(DBT* key,
                                          KEY* key_info,
                                          uchar* buff,
                                          const uchar* record,
                                          bool* has_null,
    ...
    // bytes, "d" > "D".
    DBT* ret = create_dbt_key_from_key(
        key, key_info, buff, record, has_null, true, key_length, COL_NEG_INF);
    DBUG_RETURN(ret);
}
...
//      the parameter key
//
DBT* ha_tokudb::pack_key(DBT* key,
                         uint keynr,
                         uchar* buff,
                         const uchar* key_ptr,
                         uint key_length,
                         int8_t inf_byte) {
    TOKUDB_HANDLER_DBUG_ENTER("key %p %u:%2.2x inf=%d",
                              key_ptr,
                              key_length,
    ...
    if (keynr != primary_key && !tokudb_test(hidden_primary_key)) {
        DBUG_RETURN(
            pack_ext_key(key, keynr, buff, key_ptr, key_length, inf_byte));
    }
    ...
    *buff++ = (uchar)inf_byte;
    ...
    for (; key_part != end && (int)key_length > 0; key_part++) {
        ...
        buff = pack_key_toku_key_field(
            buff, (uchar*)key_ptr + offset, key_part->field, key_part->length);
...
#if defined(TOKU_INCLUDE_EXTENDED_KEYS) && TOKU_INCLUDE_EXTENDED_KEYS
DBT* ha_tokudb::pack_ext_key(DBT* key,
                             uint keynr,
                             uchar* buff,
                             const uchar* key_ptr,
                             uint key_length,
    ...
    if (key_length > 0) {
        ...
        for (uint pk_index = 0;
            ...
            if (i < pk_next) {
                ...
                buff = pack_key_toku_key_field(buff,
                                               (uchar*)this_key_ptr,
                                               this_key_part->field,
                                               this_key_part->length);
            } else {
                buff = pack_key_toku_key_field(
                    buff, (uchar*)key_ptr, key_part->field, key_part->length);
                key_ptr += key_part->store_length;
                key_length -= key_part->store_length;
                key_part++;
            }
...
void ha_tokudb::init_hidden_prim_key_info(DB_TXN* txn) {
    ...
    if (!(share->status & STATUS_PRIMARY_KEY_INIT)) {
        ...
        error = share->key_file[primary_key]->cursor(
            share->key_file[primary_key], txn, &c, 0);
        assert_always(error == 0);
        DBT key, val;
        memset(&key, 0, sizeof(key));
...
int ha_tokudb::get_status(DB_TXN* txn) {
    ...
    if (!share->status_block) {
        error = open_status_dictionary(
            &share->status_block, share->full_table_name(), txn);
        if (error) {
            goto cleanup;
        }
    }
    ...
    if (error == DB_NOTFOUND) {
        ...
        error = write_to_status(share->status_block,
                                hatoku_new_version,
                                &share->version,
...
bool ha_tokudb::may_table_be_empty(DB_TXN* txn) {
    ...
    if (empty_scan == tokudb::sysvars::TOKUDB_EMPTY_SCAN_LR)
        error =
            tmp_cursor->c_getf_next(tmp_cursor, 0, smart_dbt_do_nothing, NULL);
    else
        error =
            tmp_cursor->c_getf_prev(tmp_cursor, 0, smart_dbt_do_nothing, NULL);
...
void ha_tokudb::start_bulk_insert(ha_rows rows) {
    TOKUDB_HANDLER_DBUG_ENTER(
        "%llu txn %p", (unsigned long long)rows, transaction);
    THD* thd = ha_thd();
    ...
    if ((rows == 0 || rows > 1) && share->try_table_lock) {
        if (tokudb::sysvars::prelock_empty(thd) &&
            ...
            if (using_ignore || is_insert_ignore(thd) ||
                ...
                int error = db_env->create_loader(db_env,
                                                  transaction,
                                                  &loader,
...
int ha_tokudb::end_bulk_insert(TOKUDB_UNUSED(bool abort)) {
    ...
    if (ai_metadata_update_required) {
        share->lock();
        error = update_max_auto_inc(share->status_block,
        ...
        if (!abort_loader && !thd_killed(thd)) {
            ...
            for (uint i = 0; i < table_share->keys; i++) {
                if (table_share->key_info[i].flags & HA_NOSAME) {
                    ...
                    error = is_index_unique(&is_unique,
                                            transaction,
                                            share->key_file[i],
                                            &table->key_info[i],
                                            DB_PRELOCKED_WRITE);
...
int ha_tokudb::is_index_unique(bool* is_unique,
                               DB_TXN* txn,
                               DB* db,
                               KEY* key_info,
                               int lock_flags) {
    ...
    *is_unique = true;
    ...
    while (error != DB_NOTFOUND) {
        ...
        int cmp;
        place_key_into_mysql_buff(
            key_info, table->record[0], (uchar*)key1.data + 1);
        place_key_into_mysql_buff(
            key_info, table->record[1], (uchar*)key2.data + 1);

        create_dbt_key_for_lookup(
            &packed_key1, key_info, key_buff, table->record[0], &has_null1);
        create_dbt_key_for_lookup(
            &packed_key2, key_info, key_buff2, table->record[1], &has_null2);
        ...
        if (!has_null1 && !has_null2) {
            ...
            if (cmp == 0) {
                ...
                place_key_into_mysql_buff(
                    key_info, table->record[0], (uchar*)key_buff + 1);
                *is_unique = false;
                break;
            }
        }
        ...
        error =
            tmp_cursor1->c_get(tmp_cursor1, &key1, &val, DB_NEXT + lock_flags);
        if (error) {
            goto cleanup;
        }
        error =
            tmp_cursor2->c_get(tmp_cursor2, &key2, &val, DB_NEXT + lock_flags);
...
int ha_tokudb::is_val_unique(bool* is_unique,
                             uchar* record,
                             KEY* key_info,
                             uint dict_index,
                             DB_TXN* txn) {
    ...
    error = share->key_file[dict_index]->cursor(share->key_file[dict_index],
                                                txn,
                                                &tmp_cursor,
                                                DB_SERIALIZABLE | DB_RMW);
    if (error) {
        goto cleanup;
    } else {
        ...
        create_dbt_key_from_key(&key_right,
                                key_info,
                                key_buff3,
                                record,
                                &has_null,
        ...
        error = tmp_cursor->c_set_bounds(
            tmp_cursor, &key, &key_right, true, DB_NOTFOUND);
        if (error == 0) {
        ...
        error = tmp_cursor->c_getf_set_range(
            tmp_cursor, flags, &key, smart_dbt_callback_lookup, &ir_info);
        if (error == DB_NOTFOUND) {
            *is_unique = true;
            error = 0;
            goto cleanup;
...
int ha_tokudb::do_uniqueness_checks(uchar* record, DB_TXN* txn, THD* thd) {
    ...
    if (share->has_unique_keys && do_unique_checks(thd, in_rpl_write_rows)) {
        ...
        for (uint keynr = 0; keynr < table_share->keys; keynr++) {
            ...
            //
            error = is_val_unique(
                &is_unique, record, &table->key_info[keynr], keynr, txn);
            if (error) {
                goto cleanup;
            }
            if (!is_unique) {
...
void ha_tokudb::test_row_packing(uchar* record, DBT* pk_key, DBT* pk_val) {
    ...
    for (uint keynr = 0; keynr < table_share->keys; keynr++) {
        ...
        tmp_num_bytes = pack_key_from_desc(
            key_buff3, row_desc, desc_size, &tmp_pk_key, &tmp_pk_val);
        ...
        if (key_is_clustering(&table->key_info[keynr])) {
            ...
            assert_always(error == 0);
            uchar* tmp_buff = NULL;
            tmp_buff = (uchar*)tokudb::memory::malloc(alloced_rec_buff_length,
                                                      MYF(MY_WME));
            ...
            tmp_num_bytes = pack_clustering_val_from_desc(
                tmp_buff, row_desc, desc_size, &tmp_pk_val);
...
int ha_tokudb::insert_row_to_main_dictionary(DBT* pk_key,
                                             DBT* pk_val,
                                             DB_TXN* txn) {
    int error = 0;
    uint curr_num_DBs = table->s->keys + tokudb_test(hidden_primary_key);
...
int ha_tokudb::insert_rows_to_dictionaries_mult(DBT* pk_key,
                                                DBT* pk_val,
                                                DB_TXN* txn,
                                                THD* thd) {
    int error = 0;
...
int ha_tokudb::write_row(uchar* record) {
    ...
    if (share->has_auto_inc && record == table->record[0]) {
        ...
        if (curr_auto_inc > share->last_auto_increment) {
            ...
            if (delay_updating_ai_metadata) {
                ...
                update_max_auto_inc(share->status_block,
                                    share->last_auto_increment);
            }
        }
        share->unlock();
    ...
    create_dbt_key_from_table(
        &prim_key, primary_key, primary_key_buff, record, &has_null);
    ...
    if (loader) {
        ...
        if (error) {
            ...
            if (error == DB_KEYEXIST && !tokudb_test(hidden_primary_key) &&
                last_dup_key != primary_key) {
                int r = share->file->getf_set(share->file,
                                              txn,
                                              DB_SERIALIZABLE,
                                              &prim_key,
...
bool ha_tokudb::key_changed(uint keynr,
                            const uchar* old_row,
                            const uchar* new_row) {
    DBT old_key;
    DBT new_key;
    ...
    create_dbt_key_for_lookup(
        &old_key, &table->key_info[keynr], key_buff3, old_row, &has_null);
    return tokudb_prefix_cmp_dbt_key(
        share->key_file[keynr], &old_key, &new_key);
}
...
int ha_tokudb::update_row(const uchar* old_row, uchar* new_row) {
    ...
    if (hidden_primary_key) {
        ...
        create_dbt_key_from_table(
            &prim_key, primary_key, key_buff, new_row, &has_null);
        create_dbt_key_from_table(
            &old_prim_key, primary_key, primary_key_buff, old_row, &has_null);
    }
    ...
    error = db_env->update_multiple(db_env,
                                    share->key_file[primary_key],
                                    txn,
                                    &old_prim_key,
                                    &old_prim_row,
...
int ha_tokudb::delete_row(const uchar* record) {
    ...
    create_dbt_key_from_table(
        &prim_key, primary_key, key_buff, record, &has_null);
    if (table_share->blob_fields) {
        if (fix_rec_buff_for_blob(max_row_length(record))) {
            error = HA_ERR_OUT_OF_MEM;
            goto cleanup;
    ...
    error = db_env->del_multiple(db_env,
                                 share->key_file[primary_key],
                                 transaction,
                                 &prim_key,
                                 &row,
...
static bool index_key_is_null(TABLE* table,
                              uint keynr,
                              const uchar* key,
                              uint key_len) {
...
int ha_tokudb::index_init(uint keynr, bool sorted) {
    ...
    cursor->c_set_check_interrupt_callback(
        cursor, tokudb_killed_thd_callback, thd);
...
int ha_tokudb::read_row_callback(uchar* buf,
                                 uint keynr,
                                 DBT const* row,
                                 DBT const* found_key) {
...
//      [in]    found_key - key used to retrieve the row
//
int ha_tokudb::read_primary_key(uchar* buf,
                                uint keynr,
                                DBT const* row,
                                DBT const* found_key) {
    TOKUDB_HANDLER_DBUG_ENTER("");
    ...
    if (keynr != primary_key && !key_is_clustering(&table->key_info[keynr])) {
        ...
        create_dbt_key_from_table(
            &last_key, primary_key, key_buff, buf, &has_null);
    }
    //
...
int ha_tokudb::index_next_same(uchar* buf, const uchar* key, uint keylen) {
    ...
    //
    create_dbt_key_from_table(
        &found_key, tokudb_active_index, key_buff3, buf, &has_null);
    cmp = tokudb_prefix_cmp_dbt_key(
        share->key_file[tokudb_active_index], &curr_key, &found_key);
    if (cmp) {
...
//      error otherwise
//
int ha_tokudb::index_read(uchar* buf,
                          const uchar* key,
                          uint key_len,
                          enum ha_rkey_function find_flag) {
    TOKUDB_HANDLER_DBUG_ENTER(
        "key %p %u:%2.2x find=%u", key, key_len, key ? key[0] : 0, find_flag);
    invalidate_bulk_fetch();
    ...
    switch (find_flag) {
        case HA_READ_KEY_EXACT: /* Find first record else error */ {
            ...
            if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_INDEX_KEY))) {
                TOKUDB_DBUG_DUMP(
                    "tokudb key=", lookup_key.data, lookup_key.size);
            }
        ...
        case HA_READ_AFTER_KEY: /* Find next rec. after key-record */
            pack_key(&lookup_key,
                     ...
                     key_len,
                     COL_POS_INF);
            error = cursor->c_getf_set_range(cursor,
                                             flags,
                                             &lookup_key,
                                             SMART_DBT_CALLBACK(key_read),
                                             &info);
            break;
        case HA_READ_BEFORE_KEY: /* Find next rec. before key-record */
            pack_key(&lookup_key,
                     ...
                     COL_NEG_INF);
            error =
                cursor->c_getf_set_range_reverse(cursor,
                                                 flags,
                                                 &lookup_key,
                                                 SMART_DBT_CALLBACK(key_read),
                                                 &info);
        ...
        case HA_READ_KEY_OR_PREV: /* Record or previous */
            ...
            error = cursor->c_getf_set_range(cursor,
                                             ...
                                             &ir_info);
            if (error == DB_NOTFOUND) {
                error = cursor->c_getf_last(
                    cursor, flags, SMART_DBT_CALLBACK(key_read), &info);
            } else if (ir_info.cmp) {
                error = cursor->c_getf_prev(
                    cursor, flags, SMART_DBT_CALLBACK(key_read), &info);
            }
            break;
...
int ha_tokudb::read_data_from_range_query_buff(uchar* buf,
                                               bool need_val,
                                               bool do_key_read) {
    ...
    // we need to get more data
        ...
        if (!need_val) {
            ...
            extract_hidden_primary_key(tokudb_active_index, &curr_key);
            error = read_primary_key(
                buf, tokudb_active_index, &curr_val, &curr_key);
        } else {
            extract_hidden_primary_key(tokudb_active_index, &curr_key);
            ...
            if (unpack_entire_row) {
                ...
                curr_val.size = val_size;
                error =
                    unpack_row(buf, &curr_val, &curr_key, tokudb_active_index);
            } else {
                if (!(hidden_primary_key &&
                ...
                for (uint32_t i = 0; i < num_var_cols_for_query; i++) {
                    ...
                    unpack_var_field(buf + field_offset(field, table),
                                     curr_pos,
                                     field_len,
                                     share->kc_info.length_bytes[field_index]);
                    curr_pos += field_len;
...
static int smart_dbt_bf_callback(DBT const* key,
                                 DBT const* row,
                                 void* context) {
    SMART_DBT_BF_INFO info = (SMART_DBT_BF_INFO)context;
    return info->ha->fill_range_query_buf(info->need_val,
                                          key,
                                          row,
                                          info->direction,
                                          info->thd,
...
int ha_tokudb::fill_range_query_buf(bool need_val,
                                    DBT const* key,
                                    DBT const* row,
                                    int direction,
                                    THD* thd,
    ...
    if (key_to_compare) {
        int cmp = tokudb_prefix_cmp_dbt_key(
            share->key_file[tokudb_active_index], key_to_compare, key);
        if (cmp) {
    ...
    *reinterpret_cast<uint32_t*>(curr_pos) = key->size;
            ...
            *reinterpret_cast<uint32_t*>(curr_pos) = row->size;
            ...
            if (read_blobs) {
                ...
                get_blob_field_info(
                    ...
                    var_field_data_ptr,
                    share->kc_info.num_offset_bytes);
                data_size = row->size - blob_offset -
    ...
    if (direction > 0) {
        ...
        int cmp = tokudb_cmp_dbt_key(
            share->key_file[tokudb_active_index], key, &right_range);
        ...
        int cmp = tokudb_cmp_dbt_key(
            share->key_file[tokudb_active_index], key, &left_range);
        error = (cmp < 0) ? 0 : TOKUDB_CURSOR_CONTINUE;
    }
cleanup:
    return error;
...
int ha_tokudb::get_next(uchar* buf,
                        int direction,
                        DBT* key_to_compare,
                        bool do_key_read) {
    int error = 0;
        ...
        if ((bytes_used_in_range_query_buff - curr_range_query_buff_offset) >
            ...
            if (doing_bulk_fetch) {
                ...
                while (bytes_used_in_range_query_buff == 0 &&
                    ...
                    if (direction > 0) {
                        error = cursor->c_getf_next(
                            cursor, flags, smart_dbt_bf_callback, &bf_info);
                    } else {
                        error = cursor->c_getf_prev(
                            cursor, flags, smart_dbt_bf_callback, &bf_info);
                    }
                ...
                if (direction > 0) {
                    error = cursor->c_getf_next(
                        cursor, flags, SMART_DBT_CALLBACK(do_key_read), &info);
                } else {
                    error = cursor->c_getf_prev(
                        cursor, flags, SMART_DBT_CALLBACK(do_key_read), &info);
                }
...
int ha_tokudb::read_range_first(const key_range* start_key,
                                ...
                                bool sorted) {
    TOKUDB_HANDLER_DBUG_ENTER(
        "%p %p %u %u", start_key, end_key, eq_range, sorted);
    int error = prelock_range(start_key, end_key);
    if (error) {
        goto cleanup;
    }
...
void ha_tokudb::position(const uchar* record) {
    ...
    if (hidden_primary_key) {
        ...
        *(uint32_t*)ref = TOKUDB_HIDDEN_PRIMARY_KEY_LENGTH;
        ...
        //
        create_dbt_key_from_table(
            &key, primary_key, ref + sizeof(uint32_t), record, &has_null);
        //
...
int ha_tokudb::info(uint flag) {
    ...
    if (flag & HA_STATUS_VARIABLE) {
        ...
        if (!(flag & HA_STATUS_NO_LOCK)) {
            error =
                txn_begin(db_env, NULL, &txn, DB_READ_UNCOMMITTED, ha_thd());
            if (error) {
                goto cleanup;
            }
...
int ha_tokudb::create_txn(THD* thd, tokudb_trx_data* trx) {
    ...
    if (thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN) &&
        ...
        if ((error =
                 txn_begin(db_env, NULL, &trx->all, txn_begin_flags, thd))) {
            goto cleanup;
        }
        TOKUDB_HANDLER_TRACE_FOR_FLAGS(
            TOKUDB_DEBUG_TXN, "created master %p", trx->all);
    ...
    if (trx->stmt) {
        TOKUDB_HANDLER_TRACE_FOR_FLAGS(
            TOKUDB_DEBUG_TXN, "warning:stmt=%p", trx->stmt);
    }
...
int ha_tokudb::external_lock(THD* thd, int lock_type) {
    TOKUDB_HANDLER_DBUG_ENTER("cmd %d lock %d %s %s",
                              thd_sql_command(thd),
                              lock_type,
    ...
    if (TOKUDB_UNLIKELY(!TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_ENTER) &&
        ...
        TOKUDB_HANDLER_TRACE("cmd %d lock %d %s %s",
                             thd_sql_command(thd),
                             lock_type,
                             lock_type_str(lock_type),
                             share->full_table_name());
...
int ha_tokudb::start_stmt(THD* thd, thr_lock_type lock_type) {
    TOKUDB_HANDLER_DBUG_ENTER("cmd %d lock %d %s",
                              thd_sql_command(thd),
                              lock_type,
                              share->full_table_name());
    ...
    if (!trx->stmt) {
        ...
        TOKUDB_HANDLER_TRACE_FOR_FLAGS(
            TOKUDB_DEBUG_TXN, "trx->stmt %p already existed", trx->stmt);
    }
...
THR_LOCK_DATA** ha_tokudb::store_lock(THD* thd,
    ...
    TOKUDB_HANDLER_DBUG_ENTER(
        "lock_type=%d cmd=%d", lock_type, thd_sql_command(thd));
    TOKUDB_HANDLER_TRACE_FOR_FLAGS(TOKUDB_DEBUG_LOCK,
                                   "lock_type=%d cmd=%d",
                                   lock_type,
                                   thd_sql_command(thd));
    ...
    *to++ = &lock;
    TOKUDB_HANDLER_TRACE_FOR_FLAGS(
        TOKUDB_DEBUG_LOCK, "lock_type=%d", lock_type);
...
static int create_sub_table(const char* table_name,
                            DBT* row_descriptor,
                            DB_TXN* txn,
                            uint32_t block_size,
    ...
    error = file->open(
        file, txn, table_name, NULL, DB_BTREE, create_flags, my_umask);
    if (error) {
        DBUG_PRINT(
            "error",
...
int ha_tokudb::remove_key_name_from_status(DB* status_block,
    ...
    //
    memcpy(status_key_info + sizeof(HA_METADATA_KEY),
           key_name,
           strlen(key_name) + 1);
    error = remove_metadata(status_block,
                            status_key_info,
...
int ha_tokudb::write_key_name_to_status(DB* status_block,
    ...
    memcpy(status_key_info + sizeof(HA_METADATA_KEY),
           key_name,
           strlen(key_name) + 1);
    error = write_metadata(status_block,
                           status_key_info,
                           sizeof(HA_METADATA_KEY) + strlen(key_name) + 1,
                           NULL,
                           0,
...
void ha_tokudb::trace_create_table_info(TABLE* form) {
    ...
    if (TOKUDB_UNLIKELY(TOKUDB_DEBUG_FLAGS(TOKUDB_DEBUG_OPEN))) {
        for (i = 0; i < form->s->fields; i++) {
            ...
            TOKUDB_HANDLER_TRACE("field:%d:%s:type=%d:flags=%x",
                                 i,
                                 field->field_name,
                                 field->type(),
                                 field->flags);
        ...
        for (i = 0; i < form->s->keys; i++) {
            ...
            TOKUDB_HANDLER_TRACE(
                "key:%d:%s:%d", i, key->name, key->user_defined_key_parts);
            uint p;
...
static uint32_t create_secondary_key_descriptor(uchar* buf,
                                                KEY* key_info,
                                                KEY* prim_key,
                                                uint hpk,
                                                TABLE* form,
...
int ha_tokudb::create_secondary_dictionary(
    ...
    //
    row_descriptor.size = create_secondary_key_descriptor(row_desc_buff,
                                                          key_info,
                                                          prim_key,
                                                          hpk,
                                                          form,
    ...
    error = create_sub_table(newname,
                             &row_descriptor,
                             txn,
                             block_size,
                             read_block_size,
...
static uint32_t create_main_key_descriptor(uchar* buf,
                                           KEY* prim_key,
                                           uint hpk,
                                           uint primary_key,
                                           TABLE* form,
...
int ha_tokudb::create_main_dictionary(
    ...
    row_descriptor.size = create_main_key_descriptor(
        row_desc_buff, prim_key, hpk, primary_key, form, kc_info);
    assert_always(row_descriptor.size <= max_row_desc_buff_size);
...
int ha_tokudb::create(const char* name,
    ...
    error = write_to_status(
        status_block, hatoku_new_version, &version, sizeof(version), txn);
    if (error) {
        goto cleanup;
    }

    capabilities = HA_TOKU_CAP;
    error = write_to_status(status_block,
                            hatoku_capabilities,
                            &capabilities,
                            sizeof(capabilities),
                            txn);
    ...
    error = write_auto_inc_create(
        status_block, create_info->auto_increment_value, txn);
    if (error) {
        goto cleanup;
    }
    ...
    error = initialize_key_and_col_info(
        form->s, form, &kc_info, hidden_primary_key, primary_key);
    if (error) {
        goto cleanup;
    }

    error =
        create_main_dictionary(name, form, txn, &kc_info, compression_method);
    if (error) {
        goto cleanup;
    }
    ...
    for (uint i = 0; i < form->s->keys; i++) {
        if (i != primary_key) {
            error = create_secondary_dictionary(name,
                                                form,
                                                &form->key_info[i],
                                                txn,
                                                &kc_info,
            ...
            error = write_key_name_to_status(
                status_block, form->s->key_info[i].name, txn);
            if (error) {
                goto cleanup;
            }
        }
...
int ha_tokudb::delete_or_rename_dictionary(const char* from_name,
    ...
    if (!is_delete) {
        ...
        new_to_name =
            (char*)tokudb::memory::malloc(new_to_name_len, MYF(MY_WME));
        if (new_to_name == NULL) {
            error = ENOMEM;
            goto cleanup;
        }
    ...
    if (is_delete) {
        ...
        error =
            db_env->dbrename(db_env, txn, new_from_name, NULL, new_to_name, 0);
    }
    if (error) {
        goto cleanup;
    }
...
int ha_tokudb::delete_or_rename_table(const char* from_name,
    ...
    while (error != DB_NOTFOUND) {
        error =
            status_cursor->c_get(status_cursor, &curr_key, &curr_val, DB_NEXT);
        if (error && error != DB_NOTFOUND) {
            error = map_to_handler_error(error);
            goto cleanup;
        }
        ...
        error = delete_or_rename_dictionary(
            from_name,
            to_name,
            (char*)((char*)curr_key.data + sizeof(HA_METADATA_KEY)),
            true,
            txn,
            is_delete);
        if (error) {
    ...
    //
    error = delete_or_rename_dictionary(
        from_name, to_name, "main", false, txn, is_delete);
    if (error) {
        goto cleanup;
    }
    ...
    //
    error = delete_or_rename_dictionary(
        from_name, to_name, "status", false, txn, is_delete);
    if (error) {
        goto cleanup;
    }
...
//
ha_rows ha_tokudb::records_in_range(uint keynr,
                                    key_range* start_key,
                                    key_range* end_key) {
    ...
    if (pleft_key && pright_key &&
        ...
        bool is_exact;
        error = kfile->keys_range64(kfile,
                                    transaction,
                                    pleft_key,
                                    pright_key,
                                    &less,
...
void ha_tokudb::init_auto_increment() {
    ...
    if (error) {
        ...
        error =
            share->status_block->get(share->status_block, txn, &key, &value, 0);
...
void ha_tokudb::get_auto_increment(ulonglong offset,
                                   ulonglong increment,
...
//      0 on success, error otherwise
//
int ha_tokudb::tokudb_add_index(TABLE* table_arg,
                                KEY* key_info,
                                uint num_of_keys,
                                DB_TXN* txn,
                                bool* inc_num_DBs,
    ...
    uint32_t loader_flags =
        loader_save_space ? LOADER_COMPRESS_INTERMEDIATES : 0;
    ...
    *modified_DBs = false;
    ...
    //
    creating_hot_index = use_hot_index && num_of_keys == 1 &&
    ...
    *modified_DBs = true;
    for (uint i = 0; i < num_of_keys; i++, curr_index++) {
        if (key_is_clustering(&key_info[i])) {
            ...
            error = initialize_col_pack_info(
                &share->kc_info, table_arg->s, curr_index);
            if (error) {
                goto cleanup;
            }
        }

        error = create_secondary_dictionary(share->full_table_name(),
                                            table_arg,
                                            &key_info[i],
                                            txn,
                                            &share->kc_info,
        ...
        error = open_secondary_dictionary(&share->key_file[curr_index],
                                          &key_info[i],
                                          share->full_table_name(),
                                          false,
                                          txn);
    ...
    if (creating_hot_index) {
        share->num_DBs++;
        *inc_num_DBs = true;
        error = db_env->create_indexer(db_env,
                                       txn,
                                       &indexer,
                                       share->file,
                                       num_of_keys,
        ...
        error = db_env->create_loader(db_env,
                                      txn,
                                      &loader,
                                      NULL,  // no src_db needed
                                      num_of_keys,
        ...
        //
        error =
            share->file->cursor(share->file, txn, &tmp_cursor, DB_SERIALIZABLE);
        if (error) {
        ...
        //
        error = tmp_cursor->c_set_bounds(tmp_cursor,
...
#ifdef HA_TOKUDB_HAS_THD_PROGRESS
                thd_progress_report(
                    thd, num_processed, (long long unsigned)share->rows);
#endif
...
//
void ha_tokudb::restore_add_index(TABLE* table_arg,
                                  uint num_of_keys,
...
int ha_tokudb::drop_indexes(uint* key_num,
    ...
    for (uint i = 0; i < num_of_keys; i++) {
        ...
        error = remove_key_name_from_status(
            share->status_block, key_info[curr_index].name, txn);
        if (error) {
            goto cleanup;
        }

        error = delete_or_rename_dictionary(share->full_table_name(),
                                            NULL,
...
int ha_tokudb::truncate_dictionary(uint keynr, DB_TXN* txn) {
    ...
    if (is_pk) {
        error = create_main_dictionary(share->full_table_name(),
                                       table,
                                       txn,
                                       &share->kc_info,
                                       compression_method);
...
int ha_tokudb::delete_all_rows_internal() {
    ...
    for (uint i = 0; i < curr_num_DBs; i++) {
        error = share->key_file[i]->pre_acquire_fileops_lock(share->key_file[i],
                                                             txn);
        if (error) {
            goto cleanup;
        }
        error =
            share->key_file[i]->pre_acquire_table_lock(share->key_file[i], txn);
        if (error) {
            goto cleanup;
        }
    }
    ...
    if (error == 0) {
        ...
        write_to_status(share->status_block,
                        hatoku_max_ai,
                        &share->last_auto_increment,
