...
int mi_repair(MI_CHECK *param, MI_INFO *info, char *name, int rep_quick,
  ...
  if (!rep_quick) {
    ...
    if (init_io_cache(&info->rec_cache, new_file,
                      ...
                      new_header_length, 1, MYF(MY_WME | MY_WAIT_IF_FULL)))
      goto err;
  }
  sort_info.info = info;
...
int mi_sort_index(MI_CHECK *param, MI_INFO *info, char *name,
  ...
  for (key = 0, keyinfo = &share->keyinfo[0]; key < share->base.keys;
       key++, keyinfo++)
    if (keyinfo->key_alg == HA_KEY_ALG_RTREE) DBUG_RETURN(0);
...
int test_if_almost_full(MI_INFO *info) {
  ...
  return mysql_file_seek(info->s->kfile, 0L, MY_SEEK_END, MYF(MY_THREADSAFE)) /
                 10 * 9 >
