...
bool EventFieldActionReplaceField::validate_field_name(
  ...
  return (class_name == kClassNameGeneral &&
         ...
         ((class_name == kClassNameTableAccess || class_name == kClassNameQuery ||
           class_name == kClassNameParse) &&
...
bool EventFieldActionReplaceField::apply(const AuditRecordFieldsList &fields,
  ...
  if (m_field_name == kFieldNameGeneralQuery &&
      std::holds_alternative<AuditRecordGeneral>(audit_record)) {
    auto *rec = std::get_if<AuditRecordGeneral>(&audit_record);
    if (rec != nullptr) {
      rec->extended_info.digest = std::move(new_value);
      return true;
    }
  } else if (m_field_name == kFieldNameQuery &&
             std::holds_alternative<AuditRecordTableAccess>(audit_record)) {
    ...
    if (rec != nullptr) {
      ...
      return true;
             ...
             std::holds_alternative<AuditRecordQuery>(audit_record)) {
    ...
    if (rec != nullptr) {
      ...
      return true;
             ...
             std::holds_alternative<AuditRecordParse>(audit_record)) {
