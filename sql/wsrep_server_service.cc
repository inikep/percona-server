...
#include "mysqld.h" /* LOCK_status, LOCK_global_system_variables */
...
extern int init_wsrep_thread(THD* thd);
extern void deinit_wsrep_thread(THD* thd);

static void init_service_thd(THD* thd, const char* thread_stack)
{
  thd->thread_stack= thread_stack;
  init_wsrep_thread(thd);
...
wsrep_create_storage_service(THD* orig_thd, const char* ctx)
{
  ...
  WSREP_DEBUG("Created storage service in %s context with thread id %u",
              ctx, thd->thread_id());
  ...
  if (view.status() == wsrep::view::primary)
  {
    if (applier)
    {
      ...
      */
      ...
      assert(wsrep_get_SE_checkpoint<wsrep::gtid>().id() == view.state_id().id());
    }
    else
    {
                                        ...
                                        const wsrep::gtid& gtid)
{
 ...
 // Wait until all prior committers have finished.
  ...
  wsrep_set_SE_checkpoint(gtid,  wsrep_local_gtid_manager.gtid(), false);
}
...
void Wsrep_server_service::debug_sync(const char* sync_point WSREP_UNUSED)
{
  DBUG_EXECUTE_IF(sync_point, {
    ...
    assert(
        !debug_sync_set_action(current_thd, action.c_str(), action.length()));
  };);
}
