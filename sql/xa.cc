...
#ifdef WITH_WSREP
  if ((gtrid_length == static_cast<long>(WSREP_XID_GTRID_LEN_V_3)) &&
      bqual_length == 0 &&
      !memcmp(data, WSREP_XID_PREFIX, WSREP_XID_PREFIX_LEN)) {
    my_xid tmp;
    memcpy(&tmp, data + WSREP_XID_SEQNO_OFFSET, sizeof(tmp));
    return tmp;
  }
#endif
  return 0;
...
#ifdef WITH_WSREP
static my_xid wsrep_order_and_check_continuity(XA_recover_txn *list, int len)
{
  ...
  WSREP_INFO("Read checkpoint %lld from storage engines", cur_seqno);
  for (int i= 0; i < len; ++i)
  {
#ifndef NDEBUG
    char buf[XIDDATASIZE * 4 + 6]; // see xid_to_str
    XID *xid= &list[i].id;
    sql_print_information("wsrep recovered xid %s", xid->xid_to_str(buf));
#endif /* NDEBUG */
    if (!wsrep_is_wsrep_xid(&list[i].id) ||
        wsrep_xid_seqno(&list[i].id) != cur_seqno + 1)
    {
  ...
  if (hton->state == SHOW_OPTION_YES && hton->recover) {
    while (
      ...
      */
	  ...
	  wsrep_xid_seqno(&info->list[i].id) : info->list[i].id.get_my_xid();
#else
