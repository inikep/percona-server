...
void
Config_state::set(const wsrep::view& view)
{
...
void
Config_state::set(enum wsrep::server_state::state status)
{
    WSREP_DEBUG("Switching status from %d to %d", status_, status);
    assert(status_ != status || status == wsrep::server_state::s_connected);
...
thd::thd (bool won) : init(), ptr(new THD(0))
{
  if (ptr)
  {
    ptr->thread_stack= (const char*) &ptr;
    wsrep_assign_from_threadvars(ptr);
    wsrep_store_threadvars(ptr);
    ptr->variables.option_bits&= ~OPTION_BIN_LOG; // disable binlog
    ptr->variables.wsrep_on = won;
    ptr->security_context()->set_master_access(~(ulong)0);
    lex_start(ptr);
  }
}
...
size_t wsrep_guess_ip (char* buf, size_t buf_len)
{
  ...
  {
    std::string bind_addr(my_bind_addr_str);
    ...
    bool unused;
    unsigned int const ip_type= wsrep_check_ip(bind_addr.c_str(), &unused);
    ...
    if (INADDR_NONE == ip_type) {
      ...
      strncpy(buf, bind_addr.c_str(), buf_len);
      ret= strlen(buf);
      goto done;
    }
  }
  ...
  if (wsrep_node_address && wsrep_node_address[0] != '\0') {
    ...
    assert(buf_len >= addr.get_address_len());
