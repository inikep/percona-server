...
static int open_table(THD* thd,
  ...
  *table= NULL;
  ...
  TABLE_LIST tables(schema_name->str, table_name->str, lock_type);
  uint flags= (MYSQL_OPEN_IGNORE_GLOBAL_READ_LOCK |
               MYSQL_LOCK_IGNORE_GLOBAL_READ_ONLY |
               MYSQL_OPEN_IGNORE_FLUSH |
               MYSQL_LOCK_IGNORE_TIMEOUT);

  if (!open_n_lock_single_table(thd, &tables, tables.lock_descriptor().type, flags)) {
    close_thread_tables(thd);
    return (1);
  }
...
static int scan_member(TABLE* table,
                       const Wsrep_id& cluster_uuid MY_ATTRIBUTE((unused)),
...
static int init_for_index_scan(TABLE* table, const uchar* key,
  ...
  if ((error= table->file->ha_index_init(table->s->primary_key, true))) {
    WSREP_DEBUG("Failed to init table for index scan: %d", error);
    return error;
  }
  ...
  case HA_ERR_GENERIC:
    WSREP_DEBUG("init_for_index_scan success or warning %d", error);
    break;
  case HA_ERR_LOCK_DEADLOCK:
    WSREP_DEBUG("init_for_index_scan deadlock failure %d", error);
...
static int delete_all_from_table(THD *thd, const std::string &table_name) {
  TABLE *table;
  init_stmt(thd);
  int err;
  if ((err = open_for_write(thd, table_name.c_str(), &table))) {
    trans_rollback_stmt(thd);
    return err;
  }
  err = init_for_scan(table);
  if (!err) {
    while ((err = next_record(table)) == 0) {
...
int Wsrep_schema::store_view(THD* thd, const Wsrep_view& view)
{
  ...
  */
  if ((error =
           Wsrep_schema_impl::delete_all_from_table(thd, cluster_table_str))) {
    WSREP_WARN("Failed to clear table %s: %d", cluster_table_str.c_str(),
               error);
    return 1;
  }
  if ((error =
           Wsrep_schema_impl::delete_all_from_table(thd, members_table_str))) {
    WSREP_WARN("Failed to clear table %s: %d", members_table_str.c_str(),
               error);
    return 1;
  }
  /*
...
Wsrep_view Wsrep_schema::restore_view(THD* thd, const Wsrep_id& own_id) const {
  ...
  /* if we arrived here by SET wsrep_start_position command,
     then lock_tables has been called already in parser, but with empty table list
     However, THD is marked as lock tables is completed, and this will fire
     an assert later when wsrep_schema tables will be locked
  */
                           ...
                           wsrep::seqno          seqno)
{
  ...
  */
                                                     ...
                                                     key_map)))
  {
    if (error == HA_ERR_END_OF_FILE || error == HA_ERR_KEY_NOT_FOUND)
    {
      WSREP_INFO("Record not found in %s.%s:trx %llu, seqno %lld, error %d",
                 frag_table->s->db.str,
                 frag_table->s->table_name.str,
                 transaction_id.get(),
                 seqno.get(),
      ...
      /*
	handler index scan failed for true error, probably BF conflict
	have to rollback the transaction and reset table level locks
       */
      WSREP_DEBUG("init_for_index_scan failure %d", error);
      ret = error;
  ...
  else if ((error = Wsrep_schema_impl::delete_row(frag_table)))
  {
    WSREP_DEBUG("delete_row failure %d", error);
    ret= 1;
  }

  if (error != HA_ERR_LOCK_DEADLOCK) Wsrep_schema_impl::end_index_scan(frag_table);
  thd->wsrep_skip_locking= false;
  return ret;
}
                                     ...
                                     const std::vector<wsrep::seqno>& fragments)
{
       ...
       i != fragments.end(); ++i)
  {
    ...
    {
      Wsrep_schema_impl::thd_context_switch thd_context_switch_back(&thd, orig_thd);
