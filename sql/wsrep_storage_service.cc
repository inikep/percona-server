...
#include "binlog.h" /* register_binlog_handler() */
                                  ...
                                  const wsrep::ws_meta& ws_meta)
{
  ...
  /* MySQL-wsrep only: Register binlog handler to make commit go through
     binlog group commit. */
  m_thd->binlog_setup_trx_data();
  register_binlog_handler(m_thd, true);
...
{
  DBUG_PRINT("info", ("Wsrep_storage_service::store_globals(%u, %p)",
                      m_thd->thread_id(), m_thd));
...
{
  DBUG_PRINT("info", ("Wsrep_storage_service::reset_globals(%u, %p)",
                      m_thd->thread_id(), m_thd));
