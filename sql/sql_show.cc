...
class Fill_authinfo_list : public Do_THD_Impl {
 ...
 public:
  Fill_authinfo_list(THD *thd, Table_ref *tables) noexcept
      : m_thd(thd),
...
int fill_schema_authinfo(THD *thd, Table_ref *tables, Item *) {
...
static int fill_slave_db_load(THD *thd, Table_ref *tables, Item *) {
...
static bool store_socket_diag_record(TABLE *table, THD *thd, THD *tmp,
  ...
  /* STATE */
  if ((val = thread_state_info(thd, tmp))) {
    table->field[i]->store(val, strlen(val), cs);
  ...
  if (si) {
    ...
    /* SLAVE_UUID */
    String slave_uuid;
    if (get_replica_uuid(si->thd, &slave_uuid, false)) {
...
class Fill_fill_socket_diag_slaves : public Do_THD_Impl {
 ...
 public:
  Fill_fill_socket_diag_slaves(THD *executing_thd, Table_ref *tables,
...
int fill_socket_diag_slaves(THD *thd, Table_ref *tables, Item *) {
...
int fill_rbr_bi_inconsistencies(THD *thd, Table_ref *tables, Item *) {
...
int fill_db_applied_hlc(THD *thd, Table_ref *tables, Item *) {
...
int fill_failure_injection_points(THD *thd, Table_ref *tables, Item *) {
  auto table = tables->table;
