...
static Sys_var_ulong Sys_log_error_verbosity(
    ...
    VALID_RANGE(1, 4), DEFAULT(2), BLOCK_SIZE(1), NO_MUTEX_GUARD, NOT_IN_BINLOG,
...
static bool slave_check_before_image_consistency_update(sys_var *, THD *,
  ...
  if (opt_slave_check_before_image_consistency ==
      Log_event::enum_check_before_image_consistency::BI_CHECK_OFF) {
...
static Sys_var_enum slave_compression_lib_enum(
    ...
    GLOBAL_VAR(opt_replica_compression_lib), CMD_LINE(OPT_ARG),
...
static bool check_outside_transaction(sys_var *self, THD *thd, set_var *) {
  if (thd->in_active_multi_stmt_transaction()) {
    my_error(ER_VARIABLE_NOT_SETTABLE_IN_TRANSACTION, MYF(0), self->name.str);
    return true;
  }
...
static bool check_enable_binlog_hlc(sys_var * /* self */, THD * /* thd */,
  ...
  if (global_gtid_mode.get() != Gtid_mode::ON && new_enable_binlog_hlc)
...
static bool validate_enable_raft(sys_var * /*self */, THD *, set_var *var) {
  ...
  if (enable_raft) {
    ...
    assert(channel_map.get_num_instances(true) == 1);
...
static Sys_var_transaction_read_only Sys_tx_read_only(
...
static Sys_var_bool Sys_write_stats_eligible_only(
...
static Sys_var_bool Sys_response_attrs_contain_client_stats(
    "response_attrs_contain_client_stats",
    "If this is enabled, then the stats of the query as well as client "
    "are sent back "
    "to clients as part of OK packet in session response attribute. Client "
    "stats is sent as a key-value pair - 'client_stats' is the key and the "
    "value is the stringified json which has information about cpu/delay.",
    SESSION_VAR(response_attrs_contain_client_stats), CMD_LINE(OPT_ARG),
    DEFAULT(false));
...
static Sys_var_bool Sys_use_mdl_mutex(
    "use_mdl_mutex",
